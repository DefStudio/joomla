...
$txt   = 'krow' . $this->row . ' ' . 'krow' . $this->row . $this->escape($category->class_sfx);

if ($this->topic->ordering)
{
	$txt =  'krow' . $this->row . '-stickymsg krow' . $this->row . '-stickymsg-red';
}

if ($this->topic->hold == 1)
{
	$txt .= 'krow' . $this->row . 'unapproved krow' . $this->row . 'unapproved';
}
else
{
	if ($this->topic->hold)
	{
		$txt .= 'krow' . $this->row . 'deleted krow' . $this->row . 'deleted';
	}
}

if ($this->topic->moved_id > 0)
{
	$txt .= 'krow' . $this->row . 'moved krow' . $this->row .  'moved';
}
...
<tr class="<?php echo $txt;?>">
	...
	<?php if ($topic->unread) : ?>
		...
		<?php echo $this->getTopicLink($topic, 'unread', $topic->getIcon($topic->getCategory()->iconset), $this->escape($topic->subject), KunenaTemplate::getInstance()->tooltips(), $category, true, true); ?>
	<?php else :  ?>
	<td class="kcol-mid kcol-ktopicicon hidden-phone">
		<?php echo $this->getTopicLink($topic, null, $topic->getIcon($topic-> getCategory()->iconset), $this->escape($topic->subject), KunenaTemplate::getInstance()->tooltips(), $category, true, false); ?>
		<?php endif;?>
	</td>
	...
	<td class="kcol-mid kcol-ktopiclastpost">
		<div class="klatest-post-info">
			...
			<?php endif; ?>
			<span class="ktopic-latest-post"><?php echo $this->getTopicLink($this->topic, 'last', JText::_('COM_KUNENA_GEN_LAST_POST'), null, KunenaTemplate::getInstance()->tooltips(), $category, false, true); ?>
